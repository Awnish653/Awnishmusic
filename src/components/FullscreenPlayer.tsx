...
export const FullscreenPlayer: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-zinc-950 flex flex-col justify-between overflow-hidden animate-fade-in">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-b from-zinc-950/70 via-zinc-950/90 to-zinc-950 pointer-events-none" />
      ...
      <div className="relative z-10 flex items-center justify-between p-4 sm:p-6 border-b border-white/5">
        ...
        <div className="flex items-center gap-1 p-1 bg-black/40 backdrop-blur-md rounded-full border border-white/10">
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded-full transition ${
              activeTab === 'player' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
            Now Playing
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded-full transition ${
              activeTab === 'queue' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
            Queue ({queue.length})
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded-full transition ${
              activeTab === 'related' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
            Related
          </button>
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] px-2 py-0.5 rounded-full bg-indigo-950 text-indigo-300 border border-indigo-500/20 font-mono">
            {audioQuality}
          </span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-6 py-4 flex flex-col justify-center max-w-lg mx-auto w-full">
        {activeTab === 'player' && (
          <div className="flex flex-col items-center my-auto">
            ...
            <div className="relative w-64 h-64 sm:w-80 sm:h-80 rounded-3xl overflow-hidden shadow-2xl shadow-black/80 ring-1 ring-white/10 mb-8">
              <img
              ...
              />
              {isPlaying && (
                <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-md flex items-center gap-1.5 border border-white/10">
            ...
            <div className="flex items-center justify-between w-full mb-6">
              <div className="flex flex-col min-w-0 pr-4">
                <h2 className="text-xl sm:text-2xl font-bold text-white truncate" title={currentSong.title}>
                  {currentSong.title}
                </h2>
                <p className="text-sm text-zinc-400 truncate mt-1" title={currentSong.artist}>
                  {currentSong.artist}
                </p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className={`p-3 rounded-full transition ${
                    liked ? 'text-rose-500 bg-rose-500/10' : 'text-zinc-400 hover:text-white bg-white/5'
                  }`}
                >
                ...
                <button
                  ...
                  className="p-3 rounded-full text-zinc-400 hover:text-white bg-white/5 transition"
                >
            ...
            <div className="flex items-center justify-between w-full px-4">
              ...
              <button
                ...
                className="w-16 h-16 rounded-full bg-white text-black flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition"
              >
                {isLoading ? (
      ...
      {/* Bottom Dismiss Tip */}
      <div className="relative z-10 p-3 text-center text-[11px] text-zinc-600 border-t border-white/5">
